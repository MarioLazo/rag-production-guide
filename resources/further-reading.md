# 📚 Further Reading

> **Curated resources to deepen your RAG knowledge**

---

## 📖 Essential Reading

### Foundational Papers

| Paper | Why It Matters |
|-------|---------------|
| [Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks](https://arxiv.org/abs/2005.11401) (NeurIPS 2020) | The original RAG paper that established the paradigm |
| [Lost in the Middle](https://arxiv.org/abs/2307.03172) (TACL 2024) | Critical research on LLM attention patterns in long contexts |
| [RAGAS: Automated Evaluation of RAG](https://arxiv.org/abs/2309.15217) (EACL 2024) | Foundation for RAG evaluation methodology |

### Must-Read Blog Posts

| Post | Author | Key Insight |
|------|--------|-------------|
| [Contextual Retrieval](https://www.anthropic.com/news/contextual-retrieval) | Anthropic | 49-67% retrieval improvement technique |
| [RAG Isn't Dead, It's Just Rarely Built Right](https://towardsdatascience.com/) | TMLS | Why 80% of RAG projects fail |
| [Patterns for Building LLM-based Systems](https://eugeneyan.com/writing/llm-patterns/) | Eugene Yan | Comprehensive patterns overview |

---

## 🎓 Learning Paths

### Beginner (Start Here)
1. [Microsoft: Generative AI for Beginners](https://github.com/microsoft/generative-ai-for-beginners) — 21-lesson course
2. [LangChain RAG Tutorial](https://python.langchain.com/docs/tutorials/rag/) — Hands-on introduction
3. [Pinecone Learning Center](https://www.pinecone.io/learn/) — Vector search fundamentals

### Intermediate
1. [LlamaIndex Documentation](https://docs.llamaindex.ai/) — Data framework deep dive
2. [Anthropic Cookbook](https://github.com/anthropics/anthropic-cookbook) — Advanced techniques
3. [AWS Bedrock Samples](https://github.com/aws-samples/amazon-bedrock-samples) — Production patterns

### Advanced
1. [GraphRAG Paper](https://arxiv.org/abs/2404.16130) — Microsoft's knowledge graph approach
2. [Self-RAG Paper](https://arxiv.org/abs/2310.11511) — Self-reflective retrieval (ICLR 2024)
3. [Adaptive-RAG Paper](https://arxiv.org/abs/2403.14403) — Query complexity routing (NAACL 2024)

---

## 🎥 Video Resources

### Conference Talks
| Talk | Event | Topic |
|------|-------|-------|
| [Building Production RAG](https://www.youtube.com/results?search_query=building+production+rag) | Various | Search for recent talks |
| [AWS re:Invent RAG Sessions](https://www.youtube.com/results?search_query=aws+reinvent+rag) | AWS re:Invent | Enterprise RAG patterns |
| [Google Cloud Next AI Sessions](https://www.youtube.com/results?search_query=google+cloud+next+rag) | Google Cloud | Vertex AI RAG |

### YouTube Channels
- **[Weights & Biases](https://www.youtube.com/@WeightsBiases)** — ML engineering content
- **[MLOps Community](https://www.youtube.com/@MLOpscommunity)** — Production ML discussions
- **[AI Engineer](https://www.youtube.com/@aiaboratory)** — Technical deep dives

---

## 📰 Stay Updated

### Newsletters
| Newsletter | Focus |
|------------|-------|
| [The Batch](https://www.deeplearning.ai/the-batch/) | Andrew Ng's weekly AI news |
| [Last Week in AI](https://lastweekin.ai/) | Comprehensive AI news roundup |
| [Ahead of AI](https://magazine.sebastianraschka.com/) | Sebastian Raschka's ML insights |

### Blogs to Follow
- [Anthropic Research](https://www.anthropic.com/research)
- [OpenAI Blog](https://openai.com/blog)
- [Google AI Blog](https://blog.google/technology/ai/)
- [Hugging Face Blog](https://huggingface.co/blog)
- [LangChain Blog](https://blog.langchain.dev/)

### Communities
- [r/MachineLearning](https://www.reddit.com/r/MachineLearning/) — Academic ML discussion
- [r/LocalLLaMA](https://www.reddit.com/r/LocalLLaMA/) — Open-source LLM community
- [MLOps Community Slack](https://mlops.community/) — Production ML practitioners
- [LangChain Discord](https://discord.gg/langchain) — Framework-specific help

---

## 🛠️ Tools Documentation

### Frameworks
| Tool | Documentation |
|------|---------------|
| LangChain | [python.langchain.com](https://python.langchain.com/) |
| LlamaIndex | [docs.llamaindex.ai](https://docs.llamaindex.ai/) |
| Haystack | [haystack.deepset.ai](https://haystack.deepset.ai/) |
| Semantic Kernel | [learn.microsoft.com/semantic-kernel](https://learn.microsoft.com/en-us/semantic-kernel/) |

### Vector Databases
| Database | Documentation |
|----------|---------------|
| Pinecone | [docs.pinecone.io](https://docs.pinecone.io/) |
| Weaviate | [weaviate.io/developers](https://weaviate.io/developers/weaviate) |
| Qdrant | [qdrant.tech/documentation](https://qdrant.tech/documentation/) |
| Chroma | [docs.trychroma.com](https://docs.trychroma.com/) |
| Milvus | [milvus.io/docs](https://milvus.io/docs) |

### Evaluation
| Tool | Documentation |
|------|---------------|
| RAGAS | [docs.ragas.io](https://docs.ragas.io/) |
| DeepEval | [docs.confident-ai.com](https://docs.confident-ai.com/) |
| MLflow | [mlflow.org/docs](https://mlflow.org/docs/latest/index.html) |

---

## 📊 Benchmark & Research Tracking

### Leaderboards
- [MTEB Leaderboard](https://huggingface.co/spaces/mteb/leaderboard) — Embedding model rankings
- [Open LLM Leaderboard](https://huggingface.co/spaces/HuggingFaceH4/open_llm_leaderboard) — LLM comparisons
- [Chatbot Arena](https://chat.lmsys.org/) — Human preference rankings

### Research Aggregators
- [Papers With Code - RAG](https://paperswithcode.com/task/retrieval-augmented-generation)
- [arXiv cs.CL](https://arxiv.org/list/cs.CL/recent) — Latest NLP papers
- [Semantic Scholar](https://www.semanticscholar.org/) — Academic paper search

---

## 📚 Books

| Book | Author | Focus |
|------|--------|-------|
| *Designing Machine Learning Systems* | Chip Huyen | Production ML principles |
| *Natural Language Processing with Transformers* | Tunstall et al. | Transformer fundamentals |
| *Building LLM Apps* | Various | Emerging publications |

---

## 🔗 Quick Links

### This Guide's Key Resources
- [Academic References](academic-references.md) — Peer-reviewed sources
- [Official Vendor Repos](official-vendor-repos.md) — Cloud provider samples
- [Community Repos](community-repos.md) — Open-source projects
- [Glossary](glossary.md) — Terminology reference

### External Quick Links
- [LangChain RAG Docs](https://python.langchain.com/docs/tutorials/rag/)
- [OpenAI Cookbook RAG](https://cookbook.openai.com/examples/how_to_call_functions_with_chat_models)
- [Anthropic RAG Guide](https://docs.anthropic.com/claude/docs/retrieval-augmented-generation)

---

<div align="center">

[← Back to Resources](../README.md#-resources)

</div>
